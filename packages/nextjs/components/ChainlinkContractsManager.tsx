...
import { PRICE_UPDATE_STATUS, MINT_UPDATE_STATUS, PAUSE_UPDATE_STATUS } from '~~/utils/chain-of-events/ChainlinkContractStatus';
import { AUTOMATION_UPDATE_TYPES } from '~~/utils/chain-of-events/AutomationUpdateTypes';

const ChainlinkContractManager = ({ extraAddress, onScheduleButtonClick }: {
    extraAddress: string,
    ...
    const deployContract = async () => {
        ...
        saveOrUpdateContractDetails(extraAddress, chainlinkContractAddress, `${updateType}Automation` + extraAddress, statusUpdate);
    }
    ...
    const saveOrUpdateContractDetails = async (extraAddress: string, contractAddress: string, contractName: string, status: any) => {
    ...
    function getInitialButtonState(type: any) {
        switch (type) {
            case AUTOMATION_UPDATE_TYPES.PRICE: return PRICE_UPDATE_STATUS.DEPLOY;
            case AUTOMATION_UPDATE_TYPES.MINT_LIMIT: return MINT_UPDATE_STATUS.DEPLOY;
            case AUTOMATION_UPDATE_TYPES.PAUSE: return PAUSE_UPDATE_STATUS.DEPLOY;
    ...
    function getFundState(type: any) {
        switch (type) {
            case AUTOMATION_UPDATE_TYPES.PRICE: return PRICE_UPDATE_STATUS.FUND;
            case AUTOMATION_UPDATE_TYPES.MINT_LIMIT: return MINT_UPDATE_STATUS.FUND;
            case AUTOMATION_UPDATE_TYPES.PAUSE: return PAUSE_UPDATE_STATUS.FUND;
            default: return 'UNKNOWN_STATUS';
        }
    }
    ...
    function getRegisterUpkeepState(type: any) {
        switch (type) {
            case AUTOMATION_UPDATE_TYPES.PRICE: return PRICE_UPDATE_STATUS.REGISTER_UPKEEP;
            case AUTOMATION_UPDATE_TYPES.MINT_LIMIT: return MINT_UPDATE_STATUS.REGISTER_UPKEEP;
            case AUTOMATION_UPDATE_TYPES.PAUSE: return PAUSE_UPDATE_STATUS.REGISTER_UPKEEP;
            default: return 'UNKNOWN_STATUS';
        }
    }
    ...
    function getScheduleState(type: any) {
        switch (type) {
            case AUTOMATION_UPDATE_TYPES.PRICE: return PRICE_UPDATE_STATUS.SCHEDULE;
            case AUTOMATION_UPDATE_TYPES.MINT_LIMIT: return MINT_UPDATE_STATUS.SCHEDULE;
            case AUTOMATION_UPDATE_TYPES.PAUSE: return PAUSE_UPDATE_STATUS.SCHEDULE;
            default: return 'UNKNOWN_STATUS';
        }
    }
    ...
    const getStepStatus = (stepIndex: any, currentState: any, updateType: any) => {
        const statuses = {
            [AUTOMATION_UPDATE_TYPES.PRICE]: [PRICE_UPDATE_STATUS.DEPLOY, PRICE_UPDATE_STATUS.FUND, PRICE_UPDATE_STATUS.REGISTER_UPKEEP, PRICE_UPDATE_STATUS.SCHEDULE],
            [AUTOMATION_UPDATE_TYPES.MINT_LIMIT]: [MINT_UPDATE_STATUS.DEPLOY, MINT_UPDATE_STATUS.FUND, MINT_UPDATE_STATUS.REGISTER_UPKEEP, MINT_UPDATE_STATUS.SCHEDULE],
            [AUTOMATION_UPDATE_TYPES.PAUSE]: [PAUSE_UPDATE_STATUS.DEPLOY, PAUSE_UPDATE_STATUS.FUND, PAUSE_UPDATE_STATUS.REGISTER_UPKEEP, PAUSE_UPDATE_STATUS.SCHEDULE]
        };
    
        const stepStatuses = statuses[updateType];
        return currentState === stepStatuses[stepIndex] || stepStatuses.indexOf(currentState) > stepIndex ? "step-base-100" : "step-neutral";
    }
    ...
    return (
        <div className='flex flex-col items-center'>
            <div className='font-outfit'>
                ...
                <select className="select bg-secondary rounded select-bordered" defaultValue={AUTOMATION_UPDATE_TYPES.PRICE} onChange={(e) => setUpdateType(e.target.value)}>
            ...
            {buttonState === getScheduleState(updateType) && (
                <button className="btn btn-gradient-primary rounded btn-md w-48 mt-12 mb-4" onClick={() => {
